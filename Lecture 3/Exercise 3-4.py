...
def train(model, criterion, optimizer, num_epochs: int = 10):
    train_losses = []
    val_losses = []
    accuracy_list = []
...
momentum_values = [0.00, 0.20, 0.50, 0.90, 0.99, 1.00]
...
for momentum in momentum_values:
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.005, momentum=momentum)
    
    train_loss, val_loss, accuracy = train(model=model, criterion=criterion, optimizer=optimizer, num_epochs=10)
    torch.save(model.state_dict(), "3-4-momentum" + str(momentum).replace(".","-") + ".pth")
    ...
    plt.tight_layout()
    plt.savefig("3-4_" + str(momentum) + "_plot.png")
